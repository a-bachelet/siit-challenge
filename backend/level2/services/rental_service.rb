...
require_relative 'price_rate_service'
...
class RentalService
  ...
  def compute_rentals_price
    @rentals_price ||= @rentals.map do |rental|
      ...
      duration_price = (1..duration_in_days).inject(0) do |total_price, day|
        total_price += car.price_per_day * PriceRateService.compute_price_per_day_rate(day)
      end
