...
class RentalService
  ...
  def compute_rentals_price
    @rentals_price ||= @rentals.map do |rental|
      ...
      total_price = PriceService.compute_total_price(rental.start_date, rental.end_date, car.price_per_day, rental.distance, car.price_per_km)
      commission = PriceService.compute_commission(total_price, rental.start_date, rental.end_date)

      { id: rental.id, price: total_price.to_i, commission: }
    end
  end
