...
describe RentalService do
  ...
  describe '#compute_rentals_price' do
    ...
    it 'calls the payment service to compute payment actions' do
      PriceService.stub :compute_total_price, 9999 do
        PriceService.stub :compute_commission, { total_commission: 7000, insurance_fee: 100, assistance_fee: 200, drivy_fee: 300 } do
          _(@rental_service.compute_rentals_price).must_equal [
            { id: 1, actions: [
              { who: 'driver', type: 'debit', amount: 9999 },
              { who: 'owner', type: 'credit', amount: 2999 },
              { who: 'insurance', type: 'credit', amount: 100 },
              { who: 'assistance', type: 'credit', amount: 200 },
              { who: 'drivy', type: 'credit', amount: 300 }
            ] },
            { id: 2, actions: [
              { who: 'driver', type: 'debit', amount: 9999 },
              { who: 'owner', type: 'credit', amount: 2999 },
